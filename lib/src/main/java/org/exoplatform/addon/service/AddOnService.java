...
public class AddOnService {
	...
	public final static List<Addon> ADDONS = getAddonsToDisplay();
	...
	public static final String PLF_VERSION = "4.4";
	...
	public static Addon getNodeById(String uuid) throws PathNotFoundException, RepositoryException {
...
*/
	...
	public static List<Addon> getAllAdons() {
		List<Addon> addons = new ArrayList<Addon>();
		ObjectMapper mapper = new ObjectMapper();
		try {

			addons = mapper.readValue(new FileReader(
					"/home/exo/Desktop/MarcketPlae/platform-4.4.0/addons/catalogs/19022AE867E1473C4F00A78864A5D29F.json"), new TypeReference<List<Addon>>(){});

		} catch (IOException e) {
			log.error("error when trying to parse addons catalog", e);
		}
		return addons;
	}
	public static List<Addon> getSableAddons() {
		List<Addon> stableAddons = new ArrayList<Addon>();
		for (Addon addon : getAllAdons()){
			if(!addon.isUnstable()){
				stableAddons.add(addon);
			}
		}
		return stableAddons;
	}
	public static List<Addon> getSableCompatibleAddons() {
		List<Addon> stableAddons = new ArrayList<Addon>();
		for (Addon addon : getCompatibleAddons()){
			if(!addon.isUnstable()){
				stableAddons.add(addon);
			}
		}
	...
	public static List<Addon> getLatestSatableAddons(){
		List<Addon> latestSatableAddons = new ArrayList<Addon>();
		List<Addon> stableAddons = getSableAddons();

		Collections.reverse(stableAddons);

		for (Addon addon : getSableCompatibleAddons()){
			if(!isAdded(addon,latestSatableAddons)){
				latestSatableAddons.add(addon);
			}
		}
		return latestSatableAddons;
	}

	public static List<Addon> getLatestSatableCompatibleAddons(){
		List<Addon> latestSatableAddons = new ArrayList<Addon>();
		List<Addon> stableAddons = getSableAddons();

		Collections.reverse(stableAddons);

		for (Addon addon : getSableAddons()){
			if(!isAdded(addon,latestSatableAddons)){
				latestSatableAddons.add(addon);
			}
		}
	...
	private static boolean isAdded(Addon addon, List<Addon> addons){
		for(Addon addon1 : addons){
			if(addon.getId().equals(addon1.getId())){
				return true;
			}
		}
		return false;
	}
	private static void checkAddons(List<Addon> addons){
		ProcessBuilder pb = new ProcessBuilder(ADDON_SCRIPT_LINUX, "list", "--installed");
		String outPut = "";
		...
		for(Addon  addon : addons){
			if(outPut.contains(addon.getId())){
	...
	private static List<Addon> getCompatibleAddons (){
		List<Addon> allAdons = getAllAdons();
		List<Addon> compatibleAddons = new ArrayList<Addon>();


		for (Addon addon: allAdons){
			if (addon.getCompatibility().contains(PLF_VERSION)) {
				addon.setCompatible(true);
				compatibleAddons.add(addon);
			}else{
				addon.setCompatible(false);
			}
		}
		return compatibleAddons;
	}
	public static List<Addon> getAddonsToDisplay(){
        List<Addon> addonsToDisplay = getLatestSatableCompatibleAddons();
        checkAddons(addonsToDisplay);
		return addonsToDisplay;
	}
}
